...
global $APPLICATION;
$APPLICATION->SetTitle(Loc::getMessage('DIGITMIND_TASKSETTER_RULES_PAGE_TITLE'));
...
<div class="wrapper">
    <fieldset>
        <legend><?= Loc::getMessage('DIGITMIND_TASKSETTER_TECHAPPEAL_TOADDRESS') ?></legend>
        <div id="email-techappeal-toaddress-wrapper">
            <?php foreach ($techappealToaddressRules as $ruleValue): ?>
                <input type="text" class="email-techappeal-input techappeal_toaddress" value="<?= trim($ruleValue) ?>" size="64"
                       placeholder="<?= Loc::getMessage('DIGITMIND_TASKSETTER_SUBSTR_OR_REGEX_PLACEHOLDER') ?>">
            <?php endforeach; ?>
            <input type="text" value="" class="email-techappeal-input techappeal_toaddress"
                   size="64" placeholder="<?= Loc::getMessage('DIGITMIND_TASKSETTER_SUBSTR_OR_REGEX_PLACEHOLDER') ?>">
        </div>
    </fieldset>
</div>
...
<div class="wrapper">
    <fieldset>
        <legend><?= Loc::getMessage('DIGITMIND_TASKSETTER_TECHAPPEAL_SUBJECT') ?></legend>
...
<div class="wrapper">
    ...
    <button id='save-params-button'><?= Loc::getMessage('DIGITMIND_TASKSETTER_RULES_SAVE_BUTTON') ?></button>
</div>
